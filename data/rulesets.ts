...
export const Rulesets: import("../sim/dex-formats").FormatDataTable = {
	...
	natdexmod: {
		...
		ruleset: ["+Unobtainable", "+Past", "Sketch Post-Gen 7 Moves"],
		onValidateSet(set) {
			const species = this.dex.species.get(set.species);
	...
	forcemonocolor: {
		...
		onValidateSet(set) {
			const color = this.toID(
				this.ruleTable.valueRules.get("forcemonocolor"),
			);
			let dex = this.dex;
			if (dex.gen < 5) {
				dex = dex.forGen(5);
	...
	forceteratype: {
		...
		onValidateRule(value) {
			if (this.dex.gen !== 9) {
				throw new Error(
					`Terastallization doesn't exist outside of Generation 9.`,
				);
			}
			const type = this.dex.types.get(value);
			if (!type.exists) throw new Error(`Misspelled type "${value}"`);
		...
		onValidateSet(set) {
			const type = this.dex.types.get(
				this.ruleTable.valueRules.get("forceteratype")!,
			);
			if (this.toID(set.teraType) !== type.id) {
				return [
					`${set.species} must have its Tera Type set to ${type.name}.`,
				];
			}
		},
	},
	forceselect: {
		...
		onChooseTeam(positions, pokemon, autoChoose) {
			...
			const speciesIndex = pokemon.findIndex(
				(p) => p.species.name === species.name,
			);
			if (autoChoose) {
	...
	beatupnicknamesmod: {
		...
		onBegin() {
			if (this.gen <= 4) {
				this.add(
					"rule",
					`Beat Up Nicknames Mod: Beat Up will not reveal any party members`,
				);
			}
		},
	...
	itemclause: {
		...
		onValidateRule(value) {
			...
			if (num < 1 || num > this.ruleTable.maxTeamSize) {
				throw new Error(
					`Item Clause must be between 1 and ${this.ruleTable.maxTeamSize}.`,
				);
			}
			return value;
		},
		onValidateTeam(team) {
			...
			const itemLimit = Number(
				this.ruleTable.valueRules.get("itemclause") || 1,
			);
	...
	accuracytrapclause: {
		...
		onValidateSet(set, format, setHas, teamHas) {
			...
			const accuracy = this.dex.moves
				.all()
				.filter((move) => {
					if (move.boosts?.accuracy) return move.boosts.accuracy < 0;
					return move.secondaries?.some(
						(x) =>
							x.chance === 100 &&
							x.boosts?.accuracy &&
							x.boosts.accuracy < 0,
				...
				.map((x) => x.id);
			if (
				set.moves.map(this.toID).some((x) => accuracy.includes(x)) &&
				(trapping.includes(this.toID(set.ability)) ||
					set.moves.map(this.toID).some((x) => trapping.includes(x)))
			) {
				return [
	...
	batonpassclause: {
		...
		onValidateSet(set, format, setHas) {
			...
			for (const moveId of set.moves) {
				...
				if (
					move.id === "flamecharge" ||
					(move.boosts?.spe && move.boosts.spe > 0)
				) {
					speedBoosted = true;
				}
	...
	speedpassclause: {
		...
		onBegin() {
			this.add(
				"rule",
				"Baton Pass Stat Clause: No Baton Passer may have a way to boost its Speed",
			);
		},
		onValidateTeam(team) {
			const boostingEffects = [
	...
	batonpasstrapclause: {
		...
		onValidateTeam(team, format, teamHas) {
			...
			for (const set of team) {
				...
				if (
					!set.moves.some((move) =>
						trappingMoves.includes(this.toID(move)),
					)
				)
					continue;
				const name = set.name
					...
					: set.species;
				return [
					`${name} has Baton Pass and a way to pass trapping, which is banned.`,
				];
			}
		},
	},
	...
	zmoveclause: {
		...
		onValidateSet(set) {
			...
			if (item.zMove)
				return [
					`${set.name || set.species}'s item ${item.name} is banned by Z-Move Clause.`,
				];
		},
		onBegin() {
	...
	deoxyscamouflageclausemod: {
		...
		onBegin() {
			this.add(
				"rule",
				"Deoxys Camouflage Clause Mod: Reveals the Deoxys forme when it is sent in battle.",
			);
		},
	},
	freezeclausemod: {
	...
	samecolorclause: {
		...
		onBegin() {
			this.add(
				"rule",
				"Same Color Clause: Pokémon in a team must be the same color",
			);
		},
		onValidateTeam(team) {
			let color = "";
	...
	sameletterclause: {
		...
		onValidateTeam(team) {
			...
			for (const set of team) {
				...
				if (!match) {
					return [
						`${species.name} cannot be used, as its name does not begin with a valid English letter.`,
					];
				}
				...
				if (pokemon.species.id === "rayquaza") {
					...
					if (!this.ruleTable.has("terastalclause")) {
						pokemon.canTerastallize =
							this.actions.canTerastallize(pokemon);
					}
				}
			}
		},
	...
	onemegaorteraclause: {
		effectType: "Rule",
		name: "One Mega or Tera Clause",
		desc: "Each team may use either one Mega Evolution <em>or</em> one Terastallization per battle, but not both.",
		onBegin() {
			this.add(
				"rule",
				"One Mega or Tera Clause: Each team may Mega Evolve or Terastallize once, but not both",
			);
		},
		onAfterMega(pokemon) {
			// Once a Mega Evolution happens, remove Terastallization from all allies
			for (const ally of pokemon.side.pokemon) {
				ally.canTerastallize = null;
			}
		},
		onAfterTerastallization(pokemon) {
			// Once a Terastallization happens, remove Mega Evolution from all allies
			for (const ally of pokemon.side.pokemon) {
	...
	stabmonsmovelegality: {
		...
		checkCanLearn(move, species, setSources, set) {
			const nonstandard =
				move.isNonstandard === "Past" && !this.ruleTable.has("natdexmod");
			if (
				!nonstandard &&
				!move.isZ &&
				!move.isMax &&
	...
	mimicglitchclause: {
		...
		onBegin() {
			this.add(
				"rule",
				"Mimic Glitch Clause: Pokemon that learn Assist, Copycat, Metronome, Mimic, or Transform can have any move.",
			);
		},
	},
	pomegglitchclause: {
		...
		onBegin() {
			this.add(
				"rule",
				"Pomeg Glitch Clause: Gen 3 Pokémon at level 5+ can have any of their level-up moves.",
			);
		},
	},
	overflowstatmod: {
	...
	eventmovesclause: {
		...
		onValidateSet(set) {
			if (!set.moves) return;
			const moveSources: NonNullable<Learnset["learnset"]> =
				Object.fromEntries(set.moves.map((move) => [this.toID(move), []]));
			...
			for (const move of set.moves) {
				...
				if (
					sources?.length &&
					sources.every((learned) => learned.includes("S"))
				) {
					problems.push(
	...
	maxtotallevel: {
		...
		onChooseTeam(positions, pokemon, autoChoose) {
			if (autoChoose) {
				return [...pokemon.keys()].sort(
					(a, b) => pokemon[a].level - pokemon[b].level,
				);
			}
			let totalLevel = 0;
	...
	tiershiftmod: {
		...
		onModifySpecies(species, target, source, effect) {
			...
			if (target && !isNatDex) {
				if (
					this.toID(target.set.item) === "lightclay" &&
					tiers.indexOf(tier) > tiers.indexOf("rubl")
				)
					tier = "rubl";
				if (
					this.toID(target.set.item) === "quickclaw" &&
					tiers.indexOf(tier) > tiers.indexOf("nubl")
				)
					tier = "nubl";
				if (
					this.toID(target.set.ability) === "drought" &&
					tiers.indexOf(tier) > tiers.indexOf("nubl")
				)
					tier = "nubl";
				if (
					this.toID(target.set.item) === "damprock" &&
					tiers.indexOf(tier) > tiers.indexOf("publ")
				)
					tier = "publ";
				if (
					this.toID(target.set.item) === "unburden" &&
					tiers.indexOf(tier) > tiers.indexOf("zubl")
				)
					tier = "zubl";
			}
		...
		onValidateTeam(team) {
			...
			for (const set of team) {
				...
				if (
					...
					(this.ruleTable.isRestricted("ability:powerconstruct") &&
	...
	proteanpalacemod: {
		...
		onBegin() {
			this.add(
				"rule",
				"Protean Palace Mod: Pok\u00e9mon become the type of the move they use.",
			);
		},
	...
	allowedpokemoves: {
		...
		onValidateRule(value) {
			...
			if (num > this.ruleTable.maxMoveCount || num < 1) {
				throw new Error(
					`Allowed Pokemoves must be between 1 and ${this.ruleTable.maxMoveCount}.`,
				);
			}
			return value;
		},
	...
	uniquepokemoves: {
		...
		onValidateRule(value) {
			...
			if (num > this.ruleTable.maxMoveCount || num < 1) {
				throw new Error(
					`Unique Pokemoves must be between 1 and ${this.ruleTable.maxMoveCount}.`,
				);
			}
			return value;
		},
		onValidateTeam(team, format, teamHas) {
			...
			const problems: string[] = [];
			const uniquePokemoves = Number(
				this.ruleTable.valueRules.get("uniquepokemoveclause") || 1,
			);
	...
	ferventimpersonationmod: {
		...
		onValidateSet(set) {
			...
			if (
				(this.toID(set.name) !== species.id &&
					this.toID(set.name) !== impersonation.id) ||
				(impersonation.isNonstandard &&
					!(
						rt.has(
							`+pokemontag:${this.toID(impersonation.isNonstandard)}`,
						) ||
	...
	badnboostedmod: {
		...
		onBegin() {
			this.add(
				"rule",
				"Bad 'n Boosted Mod: All of a Pokemon's base stats below 70 are doubled.",
			);
		},
		onModifySpeciesPriority: 2,
		onModifySpecies(species) {
			...
			if (target.hasAbility("illusion") && !target.m.revealed) {
				...
				if (gen >= 3) {
					...
					if (
						species.abilities["1"] &&
						(gen >= 4 ||
							Dex.abilities.get(species.abilities["1"]).gen === 3)
					) {
						buf += `<span class="col twoabilitycol">${species.abilities["0"]}<br />${species.abilities["1"]}</span>`;
					} else {
						buf += `<span class="col abilitycol">${species.abilities["0"]}</span>`;
	...
	rebalancelevels: {
		...
		onBegin() {
			const rebalanceLevel = (
				...
				let newStats: StatsTable = this.spreadModify(
					newSpecies.baseStats,
					set,
				);
				...
				statRatioTotal += statRatios.power = Math.log(
					(oldStats.atk + oldStats.spa) / (newStats.atk + newStats.spa),
				);
				...
				statRatioTotal += statRatios.bulk =
					Math.log(
						(oldStats.hp * oldStats.def * oldStats.spd) /
							(oldStats.def + oldStats.spd),
					) -
					Math.log(
						(newStats.hp * newStats.def * newStats.spd) /
							(newStats.def + newStats.spd),
					);
				// calculate the ratio of the new speed to the old stats' speed at half weight
				statRatioTotal += statRatios.speed =
					Math.log(oldStats.spe / newStats.spe) / 2;
				...
				let newLevel = Math.min(
					Math.floor(Math.E ** (statRatioTotal / 5) * oldLevel),
					this.ruleTable.maxLevel,
				);
			...
			for (const poke of this.getAllPokemon()) {
				...
				poke.set.level = (poke as any).level = rebalanceLevel(
					oldSpecies,
					poke.set,
					newSpecies,
				);
		...
		onValidateRule() {
			if (!this.format.team)
				throw new Error(
					"The Rebalance Levels rule is only intended to work with randomized teams.",
				);
			if (this.ruleTable.adjustLevel) {
				throw new Error(
					`This format's rules force Pokemon to be level ${this.ruleTable.adjustLevel}, so they can't be rebalanced.`,
				);
			}
			const speciesMods = [...this.ruleTable.keys()]
				.map((r) => this.dex.data.Rulesets[r])
				.filter((r) => r?.onModifySpecies);
