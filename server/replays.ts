...
export const replaysDB = Config.replaysdb
	? new PGDatabase(Config.replaysdb)
	: null!;
...
export const Replays = new (class {
	...
	toReplay(this: void, row: ReplayRow) {
		const replay: Replay = {
			...
			players: row.players
				.split(",")
				.map((player) =>
					player.startsWith("!") ? player.slice(1) : player,
				),
		};
	...
	async add(replay: Replay) {
		...
		return (
			replayData.id +
			(replayData.password ? `-${replayData.password}pw` : "")
		);
	}
		...
		for (let i = 0; i < length; i++) {
			password +=
				this.passwordCharacters[
					crypto.randomInt(0, this.passwordCharacters.length - 1)
				];
		}
			...
			if (args.username2) {
				...
				if (format) {
					return replays.query()`SELECT 
							p1.uploadtime AS uploadtime, p1.replay_id AS id, p1.format AS format, p1.players AS players, 
							p1.rating AS rating, p1.password AS password, p1.private AS private 
					FROM replay_players p1 INNER JOIN replay_players p2 ON p2.replay_id = p1.replay_id 
				...
				if (format) {
					return replays.query()`SELECT uploadtime, replay_id AS id, format, players, rating, password FROM replay_players 
	...
	fullSearch(term: string, page = 0): Promise<Replay[]> {
		...
		const secondPattern =
			patterns.length >= 2 ? SQL`AND log LIKE ${patterns[1]} ` : undefined;
	...
	recent() {
		...
		)`WHERE private = 0 ORDER BY uploadtime DESC LIMIT 50`.then(
			this.toReplays,
		);
	}
})();
